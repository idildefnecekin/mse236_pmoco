...
class CVRP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        '''
        self.hyper_fc1 = KANLayer(hyper_input_dim, hyper_hidden_embd_dim)
        self.hyper_fc2 = KANLayer(hyper_hidden_embd_dim, hyper_hidden_embd_dim)
        self.hyper_fc3 = KANLayer(hyper_hidden_embd_dim, self.hyper_output_dim)
        self.biasfc_1 = nn.Linear(hyper_input_dim, hyper_hidden_embd_dim, bias=False).requires_grad_(True)
        self.biasfc_1.weight.data *= 0.
        self.biasfc_2 = nn.Linear(hyper_hidden_embd_dim, hyper_hidden_embd_dim, bias=False).requires_grad_(True)
        self.biasfc_2.weight.data *= 0.
        self.biasfc_3 = nn.Linear(hyper_hidden_embd_dim, self.hyper_output_dim, bias=False).requires_grad_(True)
        self.biasfc_3.weight.data *= 0.
    ...
    def assign(self, pref):
        ...
        hyper_embd, _, _, _ = self.hyper_fc1(pref)
        #hyper_embd = hyper_embd + self.biasfc_1.weight
        hyper_embd, _, _, _ = self.hyper_fc2(hyper_embd)
        #hyper_embd = hyper_embd + self.biasfc_2.weight
        mid_embd, _, _, _ = self.hyper_fc3(hyper_embd)
        #mid_embd = mid_embd + self.biasfc_3.weight
        mid_embd = mid_embd.reshape(-1, self.hyper_output_dim)

        expected_shape = (self.embd_dim,)
        print(mid_embd.shape, expected_shape)
        if mid_embd[:1 * self.embd_dim].shape != expected_shape:
            raise ValueError("Expected shape ", expected_shape," but got", mid_embd[:1 * self.embd_dim].shape)
