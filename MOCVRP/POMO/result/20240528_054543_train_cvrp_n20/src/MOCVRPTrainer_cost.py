...
class CVRPTrainer:
    ...
    def _train_one_batch(self, batch_size):
        ...
        #pref = torch.rand([2])
        pref1 = torch.normal(mean=0.5, std=torch.tensor(0.1))
        pref2 = 1-pref1
        pref = torch.stack([pref1, pref2], dim=0)
        #pref = pref / torch.sum(pref)
        # shape of pref = (2)
        #turn shape of pref to (1, 2)
        ...
        reward = - reward
        z = torch.ones(reward.shape)*0.0#.cuda() * 0.0
        ...
        ## shape = (batch, group)
        tch_reward1 = torch.exp(tch_reward)

        tch_advantage = tch_reward1 - tch_reward1.mean(dim=1, keepdim=True)
